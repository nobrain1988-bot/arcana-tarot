...
"Past, Present and Future" means nothing if you have never read tarot. So Arcana asks what you want to know, and picks the spread for you. 49 questions across five areas:
...
전부 만들어 두었다. 플레이 콘솔에 올리기만 하면 된다.

| 항목 | 규격 | 파일 |
|---|---|---|
| 앱 아이콘 | 512 × 512 PNG | ✅ `store/icon-512.png` |
| 그래픽 이미지 | 1024 × 500 PNG | ✅ `store/feature-graphic.png` |
| 휴대전화 스크린샷 | 1080 × 1920 PNG · 5장 | ✅ `store/screenshots/` |
| 태블릿 스크린샷 | 선택 | 생략 |

**다시 만들려면** (화면을 고친 뒤):

```
npm run build && npm run preview     # 다른 창에서 띄워 둔 채
npm run shots                        # 스크린샷 5장
npm run feature                      # 그래픽 이미지
```

이미 깔려 있는 크롬을 폰 크기(360×640 @3배 = 1080×1920)로 띄워서 진짜 앱을
조작하며 찍는다. 안드로이드 스튜디오도 에뮬레이터도 필요 없다.

**스크린샷 5장의 구성** — 무엇을 보여주려고 고른 것인가:

| | 화면 | 노리는 것 |
|---|---|---|
| 1 | 시작 화면 (리더와 촛불) | 첫인상. 목록에서 눈이 멈추는 그림이다 |
| 2 | 질문 목록 | 이 앱이 무엇을 해 주는지가 한 화면에 다 보인다 |
| 3 | 카드 고르기 (5×5) | '내가 직접 고른다' — 이 앱의 핵심 경험 |
| 4 | 리딩 결과 | 카드 그림 + 자리 이름 + 해석. 실제 결과물 |
| 5 | 언어 설정 | 9개 국어를 눈으로 보여준다. 전환율에 크게 기여한다 |
